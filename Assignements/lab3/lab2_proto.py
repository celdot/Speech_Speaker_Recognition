...
def forward(log_emlik, log_startprob, log_transmat):
    ...
    for n in range(1, N):
        for j in range(M):
            log_alpha[n, j] = log_emlik[n, j] + \
                logsumexp(log_alpha[n-1] + log_transmat[:, j])
    return log_alpha
    ...
    for n in range(N-2, -1, -1):
        for i in range(M):
            backward_prob[n, i] = logsumexp(
                log_emlik[n+1] + log_transmat[i] + backward_prob[n+1])
...
def viterbi(log_emlik, log_startprob, log_transmat, forceFinalState=True):
    ...
    pred = np.zeros((N, M), dtype=int)
    log_V[0] = log_startprob + log_emlik[0]
    pred[0] = -1

    for t in range(1, N):
        for j in range(M):
            log_V[t, j] = log_emlik[t, j] + \
                np.max(log_V[t-1] + log_transmat[:, j])
            pred[t, j] = np.argmax(log_V[t-1] + log_transmat[:, j])
    ...
    for n in range(N-2, -1, -1):
        viterbi_path[n] = pred[n+1, viterbi_path[n+1]]
    viterbi_loglik = log_V[-1, best_state]

    return viterbi_loglik, viterbi_path, log_V
...
def updateMeanAndVar(X, log_gamma, varianceFloor=5.0):
    ...
    for i in range(M):
        ...
        new_var = np.sum(gamma[:, i][:, np.newaxis] *
                         (X - means[i]) ** 2, axis=0) / sum_gamma_i
